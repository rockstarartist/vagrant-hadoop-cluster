...
#Get the Ambari and HDP Repo for Centos7
...
#Install Ambari-Server, set it up, start it
yum -y install ambari-server
ambari-server setup -s
ambari-server start

#Install the Agent (Leave it as localhost in its ini file)
yum -y install ambari-agent
ambari-agent start

#Register the blueprint
curl -u admin:admin -i -H 'X-Requested-By: ambari' -X POST -d @/vagrant/test-blueprint.json http://masternode.vm:8080/api/v1/blueprints/test-blueprint

#Create Hadoop cluster
curl -u admin:admin -i -H 'X-Requested-By: ambari' -X POST -d @/vagrant/cluster-template.json http://masternode.vm:8080/api/v1/clusters/hdpdevcluster
